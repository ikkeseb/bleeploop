...
//!   modulation ([`synth`]); the per-track FX chain with its filter, stutter and feedback delay, and
//!   the shared reverb bus ([`fx`]); the reverb IR ([`reverb_ir`]); the master limiter ([`compressor`]).
//!
