/**
 * Checks compensated punch windows through the real looper dispatchers, not a formula port:
 * synthetic wet audio is delayed by the compensation production itself returns, and impulses just
 * before punch-in, inside the window and just after punch-out prove the overdub keeps exactly the
 * musical window across dub/playStop/stopAll/doubleStop/clear (`--mode=` restricts to one). A
 * trailing zero-compensation case stalls both overdub buffer allocations and checks the coded
 * punch-in head survives.
 * Run: pnpm probe overdub-window [--mode=<dub|playStop|stopAll|doubleStop|clear>] [--url=<server>]
 * Browser rig only: no native driver or physical latency is measured here.
 */
