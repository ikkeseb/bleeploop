...
| `pnpm native:engine-loopback` | Where takes land against the click on the engine (`src/debug/engine-loopback.ts`), ASIO, in engine-smoke's profile, one launch: with an interface output cabled into input 2 (`--channel=<0-based>`, default 1), at each of `--buffers=` (default `64,128,256`) a FIXED first take of the click (A), a later take of lane 1's playback (B), STOP ALL → PLAY ALL, the click again (C) and lane 1 again (D). An offset is the recorded click's onset against its beat in the committed lane PCM, less the detector's lag on the engine's ideal click; nothing is fitted. Bars: \|A\| ≤ 2 ms, spread ≤ 1 ms, drift ≤ 0.1 ms/min, the accent on beat 1, and B = 2·A, C = A, D = B within 0.1 ms; any failed bar fails the run. `--plugin=` picks the live effect (default `Pro-Q:vst3`; `none` = MIC, the input dry), `--bars=` the take length; with MIC, `--unload-first=<plugin>` loads and unloads that plugin in slot 1 first, and the take's `peak gain` (cable × slot gain) should equal a run without it. Audible clicks. |
