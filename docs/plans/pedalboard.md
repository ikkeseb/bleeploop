...
| 4 | Rig recall — **landed**: `src/audio/rig-recall.ts`, run by `src/app/boot.ts` after the scan; each slot's last plugin reloads through `selectPlugin`, never armed, and an in-flight marker turns a launch that died restoring into one skipped recall. The input channel stays the one global Audio Settings choice | M | `src/audio/`, `src/app/` | `pnpm probe rig-recall`; `pnpm native:recall`: a restart brings back both plugins and the channel, a WebView reload the plugins, all unarmed; a launch killed while restoring makes the next one skip |
| 5 | Help: first screen ordered by the promise | S | `src/ui/settings/Help.tsx` | eye lap |
