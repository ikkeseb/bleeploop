...
bridge queue's shift since the freeze. The worklet that reads the bridge ring holds its queue on one
setpoint and settles it back after any step (`worklets/plugin-pcm-source.ts`).
