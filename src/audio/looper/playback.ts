...
export function cancelOverdubSwap(i: number): void {
  const od = engineState.tracks[i]?.overdub;
  if (!od || od.timer === null) return;
  clearTimeout(od.timer);
  od.timer = null;
}

/**
 * At the next loop boundary, commit the summed overdub layer: copy the session's working copy back into the
 ...
 * cannot leave the lane OVERDUBBING with no swap left. The layer stays in the session's `buf`, so STOP
 * commits it.
 */
export function scheduleOverdubSwap(i: number, onSwapFailed: (error: unknown) => void, boundary?: number): void {
  cancelOverdubSwap(i);
  const t = engineState.tracks[i];
  const session = t.overdub;
  if (!session) return; // only an overdub in flight has a boundary to swap on
  ...
  const timer = setTimeout(() => {
    const od = t.overdub;
    if (!od || od.timer !== timer) return;
    od.timer = null;
    if (t.state !== 'OVERDUBBING') return;
    try {
      // Commit summed layer into the record buffer.
      t.record.set(od.buf.subarray(0, master), 0);
      ...
      const audioBuf = od.swapBufs[od.swapIdx];
    ...
    // No refill needed: line "t.record.set(od.buf…)" above made record and the working copy
